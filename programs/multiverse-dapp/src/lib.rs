...
// Fix: Use a valid program ID instead of 
declare_id!("7xDwUMmJTJfa1QQDmhWvEFWR7GQ28hqHeRPzdWY7nJTj");
...
pub fn create_revenue_distribution_event(
    ...
    revenue_event.timestamp = current_timestamp;
    ...
    staking_pool.last_distribution_timestamp = current_timestamp;

    // Dynamic message based on current staker count
    if staking_pool.active_stakers_count == 0 {
        msg!(
            "📦 Revenue distribution event {} created with {} tokens - funds reserved for future stakers",
            event_id,
            revenue_amount
        );
    } else {
        msg!(
            "✅ Revenue distribution event {} created with {} tokens for {} active stakers",
            event_id,
            revenue_amount,
            staking_pool.active_stakers_count
        );
    }
    ...
    pub fn distribute_tournament_revenue(
        ...
        let staking_amount = (total_funds as u128 * staking_percentage as u128 / 100) as u64; //staking amount refers to amount that will go to staking pool revenue
        ...
        if staking_amount > 0 {
            ...
            ctx.accounts.staking_pool.total_accumulated_revenue = ctx.accounts.staking_pool.total_accumulated_revenue
